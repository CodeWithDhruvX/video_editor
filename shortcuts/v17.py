...
def generate_highlighted_subtitle_ass_improved(subtitle_groups, ass_path, subtitle_color=(255, 255, 255), border_color=(0, 0, 0), font_name="Roboto Bold", font_size=18):
    """
    ENHANCED: Generate YouTube-friendly ASS subtitles with customizable colors, fonts,
...
Style: Default,{font_name},{font_size},{primary_color},&H000000FF,{outline_color},&H66000000,-1,0,0,0,100,100,0,0,3,2,1,2,50,50,40,1
...
""")
        ...
        logging.info(f"✅ Generated ASS file with {len(subtitle_groups)} styled subtitles (Font: {font_name} {font_size}pt, Color: {subtitle_color}, Border: {border_color})")
...
def generate_title_overlay_ass(ass_path, video_duration, title_text, title_font_name="Arial", title_font_size=56, title_color=(255, 255, 255), title_position=8, display_duration=None):
    """
    ENHANCED: Generate title overlay ASS with customizable font, size, color, position, and duration
    """
    try:
        # Convert RGB color to ASS BGR format
        title_color_hex = rgb_to_bgr_hex(title_color)
        
        # Calculate display duration based on video length if not specified
        if display_duration is None:
            if video_duration <= 0:
                video_duration = 10
            display_duration = min(15, max(3, video_duration * 0.2))
        
        # Position mapping (ASS alignment values)
        # 1=bottom left, 2=bottom center, 3=bottom right
        # 4=middle left, 5=middle center, 6=middle right  
        # 7=top left, 8=top center, 9=top right
        alignment = title_position
        
        # Adjust margins based on position
        margin_v = 50  # Vertical margin
        if alignment in [1, 2, 3]:  # Bottom positions
            margin_v = 100
        elif alignment in [7, 8, 9]:  # Top positions  
            margin_v = 50
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: TitleStyle,{title_font_name},{title_font_size},{title_color_hex},&H000000FF,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,3,2,{alignment},20,20,{margin_v},1
...
""")
            ...
            if clean_title:
                # Add fade in/out effects with glow
                formatted_title = f"{{\\fad(1000,1000)\\bord3\\shad2}}{clean_title.upper()}"
                f.write(f"Dialogue: 1,{format_time(0)},{format_time(display_duration)},TitleStyle,,0,0,0,,{formatted_title}\n")
                
        logging.info(f"✅ Generated title overlay: '{title_text}' ({title_font_name} {title_font_size}pt, Duration: {display_duration:.1f}s)")
...
def process_video_with_settings(input_video, output_video, background_music, subtitle_individual, subtitle_highlighted, add_title, use_auto_editor, use_transcription, progress_callback=None, min_subtitle_duration=1.5, max_words_per_subtitle=4, subtitle_color=(255, 255, 255), border_color=(0, 0, 0), music_volume=0.15, subtitle_font_name="Roboto Bold", subtitle_font_size=18, title_text="", title_font_name="Arial", title_font_size=56, title_color=(255, 255, 255), title_position=8, title_duration=None):
    """
    ENHANCED: Main processing function with customizable subtitle colors, fonts, title overlay, and music volume
    """
    ...
    try:
        ...
        logging.info(f"ENHANCED: Processing video: {video_duration:.1f}s duration, subtitle font: {subtitle_font_name} {subtitle_font_size}pt, title font: {title_font_name} {title_font_size}pt")
        ...
        # Generate subtitle files with custom colors and fonts
        if subtitle_highlighted and use_transcription and subtitle_groups:
            logging.info(f"Generating synchronized subtitle file with custom font and colors: {subtitle_font_name} {subtitle_font_size}pt")
            generate_highlighted_subtitle_ass_improved(
                subtitle_groups, temp_files['highlighted_ass'], 
                subtitle_color, border_color, subtitle_font_name, subtitle_font_size
            )
        
        # Generate title overlay with custom settings
        if add_title:
            if not title_text.strip():
                title_text = get_title_for_video(input_video)
            
            generate_title_overlay_ass(
                temp_files['title_ass'], video_duration, title_text,
                title_font_name, title_font_size, title_color, 
                title_position, title_duration
            )
        ...
        logging.info(f"✅ ENHANCED: Successfully processed with custom fonts and settings: {output_video}")
...
class VideoProcessorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Video Processor - Customizable Fonts, Colors, Titles & Music")
        ...
        # Font options
        self.font_options = [
            "Arial", "Arial Bold", "Calibri", "Calibri Bold", 
            "Roboto", "Roboto Bold", "Open Sans", "Open Sans Bold",
            "Helvetica", "Helvetica Bold", "Times New Roman", "Times New Roman Bold",
            "Verdana", "Verdana Bold", "Tahoma", "Tahoma Bold",
            "Impact", "Comic Sans MS", "Georgia", "Trebuchet MS"
        ]
        
        # Position options for title
    ...
    def choose_title_color(self):
        """Open color chooser for title color"""
        color = colorchooser.askcolor(
            initialcolor=self.rgb_to_hex(self.title_color),
            title="Choose Title Color"
        )
        if color[0]:  # If a color was selected
            self.title_color = tuple(int(c) for c in color[0])
            self.title_color_label.config(
                text=f"Title Color: RGB{self.title_color}",
                background=self.rgb_to_hex(self.title_color),
                foreground="white" if sum(self.title_color) < 400 else "black"
            )
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(options_frame, text="📋 Add Title Overlay (ENHANCED - Customizable)", variable=self.add_title_var, command=self.on_title_toggle).pack(anchor="w", pady=2)
        ...
        subtitle_font_frame.pack(fill="x", pady=(0, 10))
        
        # Subtitle font selection
        ttk.Label(subtitle_font_frame, text="Subtitle Font:").grid(row=0, column=0, sticky="w", padx=(0, 10))
        self.subtitle_font_var = tk.StringVar(value="Roboto Bold")
        subtitle_font_combo = ttk.Combobox(subtitle_font_frame, textvariable=self.subtitle_font_var, values=self.font_options, state="readonly", width=20)
        subtitle_font_combo.grid(row=0, column=1, sticky="w", padx=(0, 20))
        
        # Subtitle font size
        ttk.Label(subtitle_font_frame, text="Size:").grid(row=0, column=2, sticky="w", padx=(0, 5))
        self.subtitle_font_size_var = tk.IntVar(value=18)
        subtitle_size_spinbox = ttk.Spinbox(subtitle_font_frame, from_=12, to=48, increment=2, textvariable=self.subtitle_font_size_var, width=8)
        subtitle_size_spinbox.grid(row=0, column=3, sticky="w")
        ...
        # ENHANCED Title Overlay Customization Frame
        title_custom_frame = ttk.LabelFrame(main_frame, text="📋 Title Overlay Settings", padding=15)
        title_custom_frame.pack(fill="x", pady=(0, 10))
        
        # Custom title text input
        title_text_frame = ttk.Frame(title_custom_frame)
        title_text_frame.pack(fill="x", pady=(0, 10))
        
        ttk.Label(title_text_frame, text="Title Text:").pack(side="left", padx=(0, 10))
        self.title_text_var = tk.StringVar(value="")
        title_entry = ttk.Entry(title_text_frame, textvariable=self.title_text_var, font=("Arial", 10))
        title_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))
        ttk.Label(title_text_frame, text="(Leave empty for auto-detection)", foreground="gray", font=("Arial", 8)).pack(side="left")
        
        # Title font settings
        title_font_frame = ttk.Frame(title_custom_frame)
        title_font_frame.pack(fill="x", pady=(0, 10))
        
        # Title font selection
        ttk.Label(title_font_frame, text="Title Font:").grid(row=0, column=0, sticky="w", padx=(0, 10))
        self.title_font_var = tk.StringVar(value="Arial Bold")
        title_font_combo = ttk.Combobox(title_font_frame, textvariable=self.title_font_var, values=self.font_options, state="readonly", width=20)
        title_font_combo.grid(row=0, column=1, sticky="w", padx=(0, 20))
        
        # Title font size
        ttk.Label(title_font_frame, text="Size:").grid(row=0, column=2, sticky="w", padx=(0, 5))
        self.title_font_size_var = tk.IntVar(value=56)
        title_size_spinbox = ttk.Spinbox(title_font_frame, from_=24, to=120, increment=4, textvariable=self.title_font_size_var, width=8)
        title_size_spinbox.grid(row=0, column=3, sticky="w")
        
        # Title color and position
        title_settings_frame = ttk.Frame(title_custom_frame)
        title_settings_frame.pack(fill="x", pady=(0, 10))
        
        # Title color
        title_color_subframe = ttk.Frame(title_settings_frame)
        title_color_subframe.pack(fill="x", pady=2)
        
        ttk.Button(
            title_color_subframe, 
            text="Choose Title Color", 
            command=self.choose_title_color
        ).pack(side="left", padx=(0, 10))
        
        self.title_color_label = ttk.Label(
            title_color_subframe, 
            text=f"Title Color: RGB{self.title_color}",
            background=self.rgb_to_hex(self.title_color),
            foreground="black",
        ...
        self.title_color_label.pack(side="left", fill="x", expand=True)
        
        # Title position and duration
        title_pos_dur_frame = ttk.Frame(title_custom_frame)
        title_pos_dur_frame.pack(fill="x", pady=(0, 5))
        
        # Position
        ttk.Label(title_pos_dur_frame, text="Position:").pack(side="left", padx=(0, 10))
        self.title_position_var = tk.StringVar(value="Top Center")
        title_pos_combo = ttk.Combobox(title_pos_dur_frame, textvariable=self.title_position_var, 
                                     values=list(self.title_positions.keys()), state="readonly", width=12)
        title_pos_combo.pack(side="left", padx=(0, 20))
        
        # Duration
        ttk.Label(title_pos_dur_frame, text="Duration:").pack(side="left", padx=(0, 5))
        self.title_duration_var = tk.DoubleVar(value=0.0)  # 0 means auto
        title_duration_spinbox = ttk.Spinbox(title_pos_dur_frame, from_=0.0, to=30.0, increment=0.5, 
                                           textvariable=self.title_duration_var, width=8, format="%.1f")
        title_duration_spinbox.pack(side="left", padx=(0, 5))
        ttk.Label(title_pos_dur_frame, text="sec (0=Auto)", foreground="gray", font=("Arial", 8)).pack(side="left")
        ...
        # Info label
        info_label = ttk.Label(settings_frame, text="✅ ENHANCED: Custom fonts, colors, titles, and perfect synchronization", 
        ...
        self.status_label = ttk.Label(progress_frame, text="Ready to process videos (ENHANCED - Custom fonts, colors & titles)", foreground="green")
    ...
    def on_title_toggle(self):
        """Handle title overlay checkbox toggle - no special action needed"""
        pass
    ...
    def start_processing(self):
        ...
        self.status_label.config(text="Initializing ENHANCED processing with custom fonts and titles...", foreground="orange")
        ...
        try:
            for i, input_video in enumerate(self.input_videos, 1):
                ...
                try:
                    ...
                    # Get title duration (0 means auto)
                    title_duration = self.title_duration_var.get() if self.title_duration_var.get() > 0 else None
                    
                    # Process with ENHANCED customizable settings
                    start_time = time.time()
                    logging.info(f"Starting ENHANCED processing of {video_name} with custom fonts and titles")
                    ...
                    process_video_with_settings(
                        ...
                        subtitle_font_name=self.subtitle_font_var.get(),
                        subtitle_font_size=self.subtitle_font_size_var.get(),
                        title_text=self.title_text_var.get(),
                        title_font_name=self.title_font_var.get(),
                        title_font_size=self.title_font_size_var.get(),
                        title_color=self.title_color,
                        title_position=self.title_positions[self.title_position_var.get()],
...
def main():
    """ENHANCED main application entry point with customizable fonts, colors, and title overlay"""
    # Check dependencies
    ...
    try:
        ...
        logging.info("✅ ENHANCED Video Processor started - Custom fonts, colors, titles, and perfect synchronization")
...
root.mainloop()   # 👈 Keeps the window alive    