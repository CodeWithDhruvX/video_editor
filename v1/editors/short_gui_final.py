...
class VideoProcessorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("🎬 Video Processor Pro - Enhanced Edition with Highlighted Overlays")
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(title_frame, text="🎬 Video Processor Pro - Enhanced Edition with Highlighted Overlays",
        ...
        # New Highlighted Overlay Option
        highlight_frame = tk.Frame(subtitle_frame, bg='#f0f0f0')
        highlight_frame.pack(fill='x', padx=10, pady=8)
        self.enable_highlight_var = tk.BooleanVar(value=True)
        tk.Checkbutton(highlight_frame, text="🌟 Enable Highlighted Text Overlay at Bottom",
                      variable=self.enable_highlight_var, bg='#f0f0f0', font=("Arial", 10, "bold")).pack(anchor='w', pady=2)
        tk.Label(highlight_frame, text="Overlay Start Time (seconds):", font=("Arial", 10, "bold"), bg='#f0f0f0').pack(anchor='w', pady=2)
        self.highlight_start_var = tk.DoubleVar(value=2.0)
        highlight_start_scale = tk.Scale(highlight_frame, from_=0.0, to=10.0, resolution=0.5,
                                       orient='horizontal', variable=self.highlight_start_var, length=200)
        highlight_start_scale.pack(side='left', padx=(0, 5))
        self.highlight_start_label = tk.Label(highlight_frame, text="2.0s", font=("Arial", 10), bg='#f0f0f0')
        self.highlight_start_label.pack(side='left', padx=(5, 0))
        highlight_start_scale.configure(command=lambda val: self.highlight_start_label.config(text=f"{float(val)}s"))
    ...
    def process_videos_thread(self):
        try:
            ...
            subtitle_settings = {
                ...
                'enable_highlight': self.enable_highlight_var.get(),
                'highlight_start': self.highlight_start_var.get()
            }
    ...
    def load_config(self):
        ...
        try:
            ...
            self.enable_highlight_var.set(config.get('enable_highlight', True))
            self.highlight_start_var.set(config.get('highlight_start', 2.0))
            
            self.toggle_merge_options()  # Update UI based on merge enable
            self.toggle_word_count()     # Update UI based on subtitle mode
...
class OptimizedVideoProcessor:
    ...
    def generate_ass_subtitles_enhanced(self, content, ass_path, settings):
        ...
        try:
            ...
            enable_highlight = settings.get('enable_highlight', False)
            highlight_start = settings.get('highlight_start', 2.0)
            border_settings = ""
            if settings['enable_borders']:
                border_color = self.hex_to_ass_color(settings['border_color'])
                thickness = settings['border_thickness']
            ...
            if enable_highlight:
                border_settings += f"\nStyle: Highlight,Impact,{subtitle_size + 4},{base_ass_color},&H00FFFF00,-1,0,0,0,100,100,0,0,1,2,1,2,10,10,2"

            with open(ass_path, "w", encoding="utf-8") as f:
                f.write(f"""[Script Info]
Title: Enhanced Subtitles with Highlighted Overlays
ScriptType: v4.00+
...
""")
                ...
                # Add highlighted overlay if enabled
                if enable_highlight and content:
                    start_time = max(0, highlight_start)
                    end_time = max(start_time + 2.0, content[-1]["end"])  # Overlay for 2 seconds or until the last word
                    highlight_text = "LAMBDA FUNCTION IN PYTHON"  # Example text, can be dynamic
                    f.write(f"Dialogue: 1,{self.format_ass_time(start_time)},{self.format_ass_time(end_time)},Highlight,,0,0,0,,{{\\b1}}{highlight_text}\n")
            ...
            highlight_text = " with highlighted overlay" if enable_highlight else ""
            logging.info(f"✅ Generated enhanced ASS subtitles with {mode_text}{border_text}{highlight_text}")
        except Exception as e:
    ...
    def process_single_video(self, input_video, output_path, extra_video, background_music,
                ...
                if words:
                    try:
                        ...
                        mode_text = {
                            ...
                            "mixed": "mixed font styles"
                        }[subtitle_settings['mode']]
                        border_text = " with speech border boxes" if subtitle_settings['enable_borders'] else ""
                        highlight_text = " with highlighted overlay" if subtitle_settings.get('enable_highlight', False) else ""
                        logging.info(f"✅ Subtitles added to video with {mode_text}{border_text}{highlight_text}")
                        cmd = [
                            "ffmpeg", "-y", "-loglevel", "error", "-i", str(current_video_path),
                            "-vf", f"subtitles={ass_path}:force_style='Alignment=2'",
                            "-c:v", "libx264" if not use_gpu else "h264_nvenc",
                            "-preset", quality_preset, "-crf", "23",
                            "-c:a", "aac", "-b:a", "192k", str(final_with_subs)
                        ]
                        process = self.run_subprocess_with_timeout(cmd, timeout=1800)
                        if process is None:
                            return
                        if not os.path.exists(final_with_subs):
                            raise Exception("Subtitle overlay failed")
                        current_video_path = final_with_subs
                    except Exception as e:
                        if self.check_stop():
                            return
                        logging.warning(f"⚠️ Subtitle processing failed: {e}. Continuing without subtitles.")
                        shutil.copy2(current_video_path, final_with_subs)
                        current_video_path = final_with_subs
                if self.check_stop():
                    return
                if background_music:
                    if self.add_background_music_with_ducking(str(current_video_path), background_music, str(final_with_music), words, music_volume, enable_ducking):
                        current_video_path = final_with_music
                    else:
                        shutil.copy2(current_video_path, final_with_music)
                        current_video_path = final_with_music
                if self.check_stop():
                    return
                if extra_video:
                    self.merge_videos_fast(str(current_video_path), extra_video, str(merged_output))
                    if os.path.exists(merged_output):
                        current_video_path = merged_output
                if self.check_stop():
                    return
                output_file = Path(output_path) / f"{clean_name}_processed.mp4"
                os.makedirs(os.path.dirname(output_file), exist_ok=True)
                shutil.copy2(str(current_video_path), str(output_file))
                size_mb = os.path.getsize(output_file) / (1024 * 1024)
                logging.info(f"✅ Processed video saved: {output_file.name} (Size: {size_mb:.1f}MB)", extra={'is_status': True})
            except Exception as e:
                if self.check_stop():
                    return
        ...
        def process_all_videos(self, input_videos, extra_video, output_dir, background_music,
                            quality_preset, use_gpu, music_volume, enable_ducking,
                            enable_auto_edit, subtitle_settings):
            if not input_videos or not output_dir:
                raise ValueError("Input videos or output directory not provided")
            total_videos = len(input_videos)
            for index, video in enumerate(input_videos, 1):
                if self.check_stop():
                    break
                try:
                    self.update_progress((index - 1) / total_videos * 100)
                    output_path = os.path.join(output_dir, f"video_{index}")
                    self.process_single_video(video, output_path, extra_video, background_music,
                                            quality_preset, use_gpu, music_volume, enable_ducking,
                                            enable_auto_edit, subtitle_settings)
                except Exception as e:
                    logging.error(f"❌ Failed to process video {os.path.basename(video)}: {e}", exc_info=True)
                    continue
            self.update_progress(100)
